...
# =============================================================================
# 5. SINGLE RUN - APSM ONLY (Run 1 of 3, seed_offset=0)
# =============================================================================
seed_offset = 0  # Run 1
print(f"\n==================== APSM RUN 1/3 (seed_offset={seed_offset}) ====================")

# --- Prerequisites: RP, WS, EVPPI ---
print(f"[APSM Run 1] Computing RP and WS values...")
RP_val = SAA_ETSCI([], seed_offset)
WS_val = SAA_ETSCI(G_nodes, seed_offset) - CGI(G_nodes)
print(f"[APSM Run 1] Computing EVPPI values...")
EVPPI_i = {i: RP_val - (SAA_ETSCI([i], seed_offset) - CGI([i])) for i in G_nodes}

# --- G. APSM (Proposed) ---
print(f"[APSM Run 1] Running APSM Algorithm...")
...
# =============================================================================
print("\n" + "="*80)
print(f"RESULTS | APSM Run 1/3 (seed_offset={seed_offset}) | Setup: {ACTIVE_SETUP} | Nodes: {len(G_nodes)}")
