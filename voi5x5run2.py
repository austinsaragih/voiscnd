# -*- coding: utf-8 -*-
"""
VOI Evaluator - 5x5 Grid - Single Run 1 of 3 (seed_offset=0)
...
# =============================================================================
# 5. SINGLE RUN (Run 2 of 3, seed_offset=10)
# =============================================================================
seed_offset = 10  # Run 2
print(f"\n==================== RUN 2/3 (seed_offset={seed_offset}) ====================")

results = {}

print(f"[Run 2] Running RP Evaluation...")
st = timeit.default_timer()
RP_val = SAA_ETSCI([], seed_offset)
results['No Information (RP)'] = {'val': RP_val, 'time': timeit.default_timer() - st}

print(f"[Run 2] Running WS Evaluation...")
st = timeit.default_timer()
...
# --- C. Greedy EVPPI/Cost ---
print(f"[Run 2] Running Greedy EVPPI/Cost...")
st = timeit.default_timer()
...
print(f"[Run 2] Running Greedy Variance/Cost...")
st = timeit.default_timer()
...
# --- E. Top-5 EVPPI ---
print(f"[Run 2] Running Top-5 EVPPI...")
st = timeit.default_timer()
EVPPI_i = {i: RP_val - (SAA_ETSCI([i], seed_offset) - CGI([i])) for i in G_nodes}
...
# --- F. Bounds (Proposed) ---
print(f"[Run 2] Running Bounds Approximation...")
st = timeit.default_timer()
EVPPI_i = {i: RP_val - (SAA_ETSCI([i], seed_offset) - CGI([i])) for i in G_nodes}
...
# --- G. APSM (Proposed) ---
print(f"[Run 2] Running APSM Algorithm...")
st = timeit.default_timer()
...
results['APSM (Proposed)'] = {'val': SAA_ETSCI(S_apsm, seed_offset), 'time': timeit.default_timer() - st, 'size': len(S_apsm)}

# =============================================================================
# 6. REPORTING
# =============================================================================
print("\n" + "="*80)
print(f"RESULTS | Run 2/3 (seed_offset={seed_offset}) | Setup: {ACTIVE_SETUP} | Nodes: {len(G_nodes)}")
...
print("-" * 80)
for name in ['APSM (Proposed)', 'Deterministic EV', 'Robust Minimax', 'Greedy EVPPI/Cost', 'Greedy Variance/Cost', 'Top-5 EVPPI', 'Bounds (Proposed)']:
    r = results[name]
